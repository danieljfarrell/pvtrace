...
file = PVTDATA + "dyes/lv570.abs.txt"
abs = load_spectrum(file)
abs = Spectrum(x=abs.x, y=abs.y*1000) # "dyes/lv570.abs.txt" is normalised, so scale to resonable units of absorption coefficient [1/m].
file = PVTDATA + "dyes/lv570.ems.txt"
ems = load_spectrum(file)
blue_layer = Material(absorption_data=abs, emission_data=ems, quantum_efficiency=0.95, refractive_index=1.5)
...
lsc_mid = LSC(origin=(0,0,H+0.001), size=(L,W,H))
lsc_mid.material = CompositeMaterial([pmma, green_layer])
lsc_mid.name = "LSC MID"

lsc_top = LSC(origin=(0,0,2*(H+0.001)), size=(L,W,H))
lsc_top.material = CompositeMaterial([pmma, blue_layer])
lsc_top.name = "LSC TOP"

scene = Scene()
scene.add_object(lsc_bottom)
scene.add_object(lsc_mid)
